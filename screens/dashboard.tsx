    ...
    >
      ...
      <View style={styles.metricsGrid}>
        ...
        <MetricCard
          ...
          accent={urgentCases > 0 ? theme.danger : undefined}
          delay={80}
        />
      ...
      <Animated.View entering={FadeInDown.delay(360).duration(350)} style={styles.card}>
        <SectionHeader
          title="Processos"
          onPress={() => router.push('/(tabs)/processos')}
        />
        {cases.length === 0 ? (
      ...
      <Animated.View entering={FadeInDown.delay(420).duration(350)} style={styles.card}>
        <SectionHeader
          title="Colaboradores"
          onPress={() => router.push('/(tabs)/colaboradores')}
        />
        {employees.slice(0, 4).map(emp => (
          <TouchableOpacity
            key={emp.id}
            style={styles.empRow}
            onPress={() => router.push(`/colaborador/${emp.id}`)}
          >
...
const styles = StyleSheet.create({
  ...
  greetingLabel: {
    fontSize: 10, color: theme.gold, letterSpacing: 2,
    fontWeight: '600', marginBottom: 2,
  },
  greetingName: {
    fontSize: 22, fontWeight: '700', color: theme.white, marginBottom: 2,
  },
  ...
  metricGoldBar: {
    ...
    height: 2, backgroundColor: theme.gold, opacity: 0.5,
  },
  metricLabel: {
    fontSize: 9, color: theme.gold, letterSpacing: 1.5,
